...
- HTTP 2xx 不等于业务成功；必须按平台明确的成功码或成功字段判断，未知响应默认失败。
- 通知只输出允许展示的业务字段，不直接序列化完整响应体。
...
- 提交前运行 `python -m compileall -q scripts templates utils tests tools`。
- 提交前运行 `python -m unittest discover -s tests -p "test_*.py" -v` 和 `node --test tests/*.test.js`。
- 提交前运行 `python tools/check_repository_consistency.py`，避免脚本头、索引和文档漂移。
